...
struct Inventory_Space4: View {
    @FirestoreQuery(collectionPath: "Inventory_Space4") private var items: [InventoryItem]
    private let db = Firestore.firestore().collection("Inventory_Space4")
    
    @AppStorage("Inventory_Name4") var Inventory_Name4: String = ""
    
    var body: some View {
        VStack {
            TextField("Inventory Name", text: $Inventory_Name4)
...
struct Inventory_Space4_Previews: PreviewProvider {
