...
struct Inventory_Space5: View {
    @FirestoreQuery(collectionPath: "Inventory_Space5") private var items: [InventoryItem]
    private let db = Firestore.firestore().collection("Inventory_Space5")
    
    @AppStorage("Inventory_Name5") var Inventory_Name5: String = ""
    
    var body: some View {
        VStack {
            TextField("Inventory Name", text: $Inventory_Name5)
...
struct Inventory_Space5_Previews: PreviewProvider {
