...
struct Inventory_Space3: View {
    @FirestoreQuery(collectionPath: "Inventory_Space3") private var items: [InventoryItem]
    private let db = Firestore.firestore().collection("Inventory_Space3")
    
    @AppStorage("Inventory_Name3") var Inventory_Name3: String = ""
    
    var body: some View {
        VStack {
            TextField("Inventory Name", text: $Inventory_Name3)
